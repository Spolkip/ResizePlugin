package me.help.atlascoreresize.utils;
...
public class PlayerAttributesUtil {
    ...
    public static void applyPlayerAttributes(Player player, FileConfiguration config) {
        ...
        // Find if the player belongs to a predefined group by checking scale
        ...
        if (detectedGroup != null) {
            setPlayerGroup(player, detectedGroup, config);  // Apply group settings
        } else {
            // Apply default attribute scaling based on player's scale
    ...
    public static void setPlayerGroup(Player player, String group, FileConfiguration config) {
        ...
        // Apply all attributes using config values
